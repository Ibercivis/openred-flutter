...
import 'maps/light_track_map.dart';
...
Widget _trackTypeIconWidget(
  ...
  switch (type) {
    case 'light':
      return Icon(Icons.wb_sunny, size: size, color: effectiveColor);
    case 'radiation':
    ...
    default:
      return Icon(Icons.route, size: size, color: effectiveColor);
  }
}
...
class _TracksPageState extends State<TracksPage> {
  ...
  String _inferLocalTrackType(LocalTrackFile local) {
    return local.trackType;
  }
  ...
  Future<void> _syncLocalTrack(LocalTrackFile track) async {
    ...
    try {
      ...
      final sanitizedRawJson = Map<String, dynamic>.from(track.rawJson)
        ..remove('campaign_password')
        ..remove('campaignPassword');
      final updatedJson = {
        ...sanitizedRawJson,
  ...
  Future<void> _downloadCloudTrackToLocal(Map<String, dynamic> cloudTrack) async {
    ...
    List<Map<String, dynamic>> normalizePointsFromMaps(List<Map<String, dynamic>> items) {
      ...
      for (final m in items) {
        ...
        DateTime ts;
        final dtRaw = m['dateTime'] ?? m['datetime'] ?? m['timestamp'];
        if (dtRaw is num) {
          ts = DateTime.fromMillisecondsSinceEpoch(dtRaw.toInt() * 1000, isUtc: true);
        } else {
        ...
        final base = <String, dynamic>{
          ...
          'accuracyMeters': _toDouble(m['accuracy'] ?? m['accuracyMeters']) ?? 0.0,
        };

        if (trackType == 'radiation') {
          final cpm = _toDouble(m['cpm']);
          final cpmRelErr = _toDouble(m['cpm_error'] ?? m['cpmRelErr']);
          final dose = _toDouble(m['dose_rate'] ?? m['doseMicroSvPerHour']);
          final doseRelErr = _toDouble(m['dose_rate_error'] ?? m['doseMicroSvPerHourRelErr']);
          if (cpm != null) base['cpm'] = cpm;
          if (cpmRelErr != null) base['cpmRelErr'] = cpmRelErr;
          if (dose != null) base['doseMicroSvPerHour'] = dose;
          if (doseRelErr != null) base['doseMicroSvPerHourRelErr'] = doseRelErr;
        } else {
          final lux = _toDouble(m['lux']);
          if (lux == null) continue;
          base['lux'] = lux;
          final cct = _toDouble(m['cct']);
          final cieX = _toDouble(m['cieX']);
          final cieY = _toDouble(m['cieY']);
          final cieU = _toDouble(m['cieU']);
          final cieV = _toDouble(m['cieV']);
          final duv = _toDouble(m['duv']);
          final tint = _toDouble(m['tint']);
          if (cct != null) base['cct'] = cct;
          if (cieX != null) base['cieX'] = cieX;
          if (cieY != null) base['cieY'] = cieY;
          if (cieU != null) base['cieU'] = cieU;
          if (cieV != null) base['cieV'] = cieV;
          if (duv != null) base['duv'] = duv;
          if (tint != null) base['tint'] = tint;
          final mode = int.tryParse((m['mode'] ?? '').toString());
          if (mode != null) base['mode'] = mode;
          final temperature = int.tryParse((m['temperature'] ?? '').toString());
          if (temperature != null) base['temperature'] = temperature;
          final batteryMv = int.tryParse((m['batteryMv'] ?? '').toString());
          if (batteryMv != null) base['batteryMv'] = batteryMv;
          final rawChannels = m['channels'];
          if (rawChannels is List) {
            final ch = <double>[];
            for (final v in rawChannels) {
              if (v is num) ch.add(v.toDouble());
            }
            if (ch.isNotEmpty) base['channels'] = ch;
          }
        }
  ...
  Future<UploadMeta?> _askUploadMeta(LocalTrackFile track) async {
    ...
    int? inferredProjectId;
    final tt = _inferLocalTrackType(track).toLowerCase().trim();
    if (tt == 'radiation') {
      inferredProjectId = Config.radiationProjectId;
    } else if (tt == 'light') {
      inferredProjectId = Config.lightProjectId;
    }

    final missionsFuture = (inferredProjectId == null)
        ? Future.value({
            'success': false,
            'message': 'Unknown track type: cannot choose project to load missions.',
          })
        : _apiService.getMissions(projectId: inferredProjectId);
                      ...
                      FutureBuilder<Map<String, dynamic>>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (!ok) {
                            ...
                            final isAuth = statusCode == 401 || statusCode == 403 || msg == 'Not authenticated';
                            return Text(
                              isAuth
                                  ? 'Login required to load missions.'
                                  : (msg?.trim().isNotEmpty == true ? msg! : 'Failed to load missions.'),
                          ...
                          if (missions.isEmpty) {
                            return Text(
                              'No missions available for this project.',
                              style: TextStyle(color: Colors.grey.shade700, fontSize: 12),
                            );
                          }
                        ...
                        FutureBuilder<Map<String, dynamic>>(
                          ...
                          builder: (context, snapshot) {
                            ...
                            if (!ok) {
                              final msg = data?['message']?.toString();
                              return Text(
                                msg?.trim().isNotEmpty == true ? msg! : 'Failed to load campaigns.',
                                style: TextStyle(color: Colors.grey.shade700, fontSize: 12),
                              );
                            }
                            ...
                            if (campaigns.isEmpty) {
                              return Text(
                                'No campaigns available for this mission.',
                                style: TextStyle(color: Colors.grey.shade700, fontSize: 12),
                              );
                            }
...
class _LocalTrackDetailPageState extends State<LocalTrackDetailPage> {
  ...
  String _effectiveTrackType() => widget.track.trackType;

  String _trackTypeLabel(String type) {
    switch (type) {
      case 'light':
        return 'Light';
      case 'radiation':
  ...
  Widget build(BuildContext context) {
    ...
    final trackType = _effectiveTrackType();
...
class _TrackDetailPageState extends State<TrackDetailPage> {
  ...
  Future<void> _downloadCloudToLocal(int cloudId) async {
    ...
    if (trackType == 'radiation') {
      ...
      final raw = widget.track['points'] ?? widget.track['measurements'];
      if (raw is! List) {
        throw Exception('Cloud light track has no embedded points; backend must include points or expose a light measurements endpoint.');
      }
      final items = raw.whereType<Map>().map((m) => Map<String, dynamic>.from(m)).toList(growable: false);
      points.addAll(normalizePointsFromMaps(items));
    }
